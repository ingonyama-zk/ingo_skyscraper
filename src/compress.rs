...
use crate::sqr_logjumps_unr_2::*;
use crate::sqr_logjumps_unr_3::*;


// uses sqr_cios_opt function that is not limited to inputs < p
pub fn compress_cios_opt_1(l: [u64; 4], r: [u64; 4]) -> [u64; 4] {
...
// uses sqr_logjumps function that is not limited to inputs < p
pub fn compress_logjumps_1(l: [u64; 4], r: [u64; 4]) -> [u64; 4] {
    let sqr0 = |x| -> [u64; 4] { sqr_logjumps_unr_3(x) };
    let a = l;
    debug_assert!(overflowing_sub(l, U64_P).1); // assert \in[0P]
    debug_assert!(overflowing_sub(r, U64_P).1); // assert \in[0P]
    // stage 0
    let sqr = sqr0(l);
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    let (l, r) = (wrapping_add(r, sqr), l);
    debug_assert!(overflowing_sub(l, U64_4P).1); // assert \in[3P]
    debug_assert!(overflowing_sub(r, U64_P).1);  // assert \in[0P]
    // stage 1
    let sqr = sqr0(l);
    debug_assert!(overflowing_sub(sqr, U64_4P).1); // assert \in[3P]
    let (l, r) = (x0p_plus_sqr3p_plus_rc_eq0p(r, sqr, 0), l);
    debug_assert!(overflowing_sub(l, U64_P).1);  // assert \in[0P]
    debug_assert!(overflowing_sub(r, U64_4P).1); // assert \in[3P]
    // stage 2
    let bar = bar_u8(l);
    let (l, r) = (x3p_plus_bar0p_plus_rc_eq0p(r, bar, 1), l);
    debug_assert!(overflowing_sub(l, U64_P).1); // assert \in[0P]
    debug_assert!(overflowing_sub(r, U64_P).1); // assert \in[0P]
    // stage 3
    let bar = bar_u8(l);   
    ...
    let sqr = sqr0(l);
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    let (l, r) = (x0p_plus_sqr2p_plus_rc_eq0p(r, sqr, 4), l);
    debug_assert!(overflowing_sub(l, U64_P).1); // assert \in[0P]
    debug_assert!(overflowing_sub(r, U64_P).1); // assert \in[0P]
    // stage 6
    let bar = bar_u8(l);   
    ...
    let sqr = sqr0(l);
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    let (l, r) = (x0p_plus_sqr2p_plus_rc_eq0p(r, sqr, 7), l);
    debug_assert!(overflowing_sub(l, U64_P).1); // assert \in[0P]
    debug_assert!(overflowing_sub(r, U64_P).1); // assert \in[0P]
    // stage 9
    let sqr = sqr0(l);
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    x0p_plus_sqr2p_plus_y0p_eq0p(r, sqr, a)
}

// uses sqr_logjumps function that is limited to inputs < p
pub fn compress_logjumps_2(l: [u64; 4], r: [u64; 4]) -> [u64; 4] {
    let sqr0 = |x| -> [u64; 4] { sqr_logjumps_unr_2(x) };
    let a = l;
    debug_assert!(overflowing_sub(l, U64_P).1); // assert \in[0P]
    debug_assert!(overflowing_sub(r, U64_P).1); // assert \in[0P]
    // stage 0
    let sqr = sqr0(l);
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    let (l, r) = (x0p_plus_sqr2p_eq0p(r, sqr), l);
    debug_assert!(overflowing_sub(l, U64_P).1); // assert \in[0P]
    debug_assert!(overflowing_sub(r, U64_P).1); // assert \in[0P]
    // stage 1
    let sqr = sqr0(l);
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    let (l, r) = (x0p_plus_sqr2p_plus_rc_eq0p(r, sqr, 0), l);
    debug_assert!(overflowing_sub(l, U64_P).1); // assert \in[0P]
    debug_assert!(overflowing_sub(r, U64_P).1); // assert \in[0P]
    // stage 2
    let bar = bar_u8(l);
...
// x + sqr
pub fn x0p_plus_sqr2p_eq0p(x: [u64; 4], sqr: [u64; 4]) -> [u64; 4] {
    debug_assert!(overflowing_sub(x, U64_P).1);    // assert \in[0P]
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    let x_plus_sqr = wrapping_add(x, sqr);   // x+sqr \in[3P]
    debug_assert!(overflowing_sub(x_plus_sqr, U64_4P).1); // assert \in[3P]
    reduce_3p(x_plus_sqr)
}

// x + sqr + y
pub fn x0p_plus_sqr2p_plus_y0p_eq0p(x: [u64; 4], sqr: [u64; 4], y: [u64; 4]) -> [u64; 4] {
    debug_assert!(overflowing_sub(x, U64_P).1);    // assert \in[0P]
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    debug_assert!(overflowing_sub(y, U64_P).1);    // assert \in[0P]
    let x_plus_sqr = wrapping_add(x, sqr);   // x+sqr \in[3P]
    debug_assert!(overflowing_sub(x_plus_sqr, U64_4P).1); // assert \in[3P]
    let tmp = wrapping_add(x_plus_sqr, y); // x+sqr+y \in [4P]
    debug_assert!(overflowing_sub(tmp, U64_5P).1);   // assert \in[4P]
    // reduce to [0P]
    return reduce_4p(tmp);
}
...
// x + sqr + rc
pub fn x0p_plus_sqr3p_plus_rc_eq0p(x: [u64; 4], sqr: [u64; 4], rc_idx: usize) -> [u64; 4] {
    debug_assert!(overflowing_sub(x, U64_P).1);    // assert \in[0P]
    debug_assert!(overflowing_sub(sqr, U64_4P).1); // assert \in[3P]
    let x_plus_sqr = wrapping_add(x, sqr);   // x+sqr \in[4P]
    debug_assert!(overflowing_sub(x_plus_sqr, U64_5P).1); // assert \in[4P]
    let (tmp, b) = overflowing_sub(x_plus_sqr, _1P_MINUS_RC[rc_idx]); // x+sqr+rc \in [4P], if not underflow
    if b { // underflow
        // guaranteed to be [0P]
...
pub fn x0p_plus_sqr2p_plus_rc_eq0p(x: [u64; 4], sqr: [u64; 4], rc_idx: usize) -> [u64; 4] {
    debug_assert!(overflowing_sub(x, U64_P).1);    // assert \in[0P]
    debug_assert!(overflowing_sub(sqr, U64_3P).1); // assert \in[2P]
    let x_plus_sqr = wrapping_add(x, sqr);   // x+sqr \in[3P]
    debug_assert!(overflowing_sub(x_plus_sqr, U64_4P).1); // assert \in[3P]
    let (tmp, b) = overflowing_sub(x_plus_sqr, _1P_MINUS_RC[rc_idx]); // x+sqr+rc \in [3P], if not underflow
    if b { // underflow
        // guaranteed to be [0P]
        return wrapping_add(x_plus_sqr, RC[rc_idx]);
    } else { // [3P]
...
// x + bar + rc
pub fn x3p_plus_bar0p_plus_rc_eq0p(x: [u64; 4], bar: [u64; 4], rc_idx: usize) -> [u64; 4] {
    debug_assert!(overflowing_sub(x, U64_4P).1); // assert \in[3P]
    // assume bar\in[5P]
