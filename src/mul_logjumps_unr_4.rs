// unrolled logjumps using i1 only with daisy-chained carries and 128b additions
use crate::constants::*;


#[inline]
pub fn mul_logjumps_unr_4(a: [u64; 4], b: [u64; 4]) -> [u64; 4] {
    ...
    (r1, _) = wadd(c11hi, c11lo, r1, c); // carry is rare
    (r0, c) = wadd(c10lo, 0u64 , r0, false);
    (r1, c) = wadd(c12lo, c01hi, r1, c);
    (r2, _) = wadd(0u64 , c12hi, r2, c); // carry is rare

    let r00 = r0 as u64;
    let (ir000hi, ir000lo) = mult(r00, U64_I1[0]);
    let (ir001hi, ir001lo) = mult(r00, U64_I1[1]);
    let (ir002hi, ir002lo) = mult(r00, U64_I1[2]);
    let (ir003hi, ir003lo) = mult(r00, U64_I1[3]);

    (r0, c) = wadd(ir000lo, 0u64 , r0, false);
    (r1, c) = wadd(c21lo  , c10hi, r1, c);
    (r2, _) = wadd(0u64   , c21hi, r2, c); // carry is rare

    let r01 = (r0 >> 64) as u64;
    let (ir010hi, ir010lo) = mult(r01, U64_I1[0]);
    let (ir011hi, ir011lo) = mult(r01, U64_I1[1]);
    let (ir012hi, ir012lo) = mult(r01, U64_I1[2]);
    let (ir013hi, ir013lo) = mult(r01, U64_I1[3]);
    ...
    (r3, _) = wadd(0u64   , ir013hi, r3, c);
    (r1, c) = wadd(ir002lo, ir000hi, r1, false);
    (r2, c) = wadd(c22hi  , c22lo  , r2, c);
    r3 = r3.wrapping_add(c as u128);  
    (r1, c) = wadd(ir001hi, ir001lo, r1, false);
    (r2, c) = wadd(0u64   , ir002hi, r2, c);
    r3 = r3.wrapping_add(c as u128);  
    (r1, c) = wadd(ir010hi, ir010lo, r1, false);
    (r2, c) = wadd(ir003hi, ir003lo, r2, c);
    r3 = r3.wrapping_add(c as u128);  
    (r1, c) = wadd(ir011lo, 0u64   , r1, false);
    (r2, c) = wadd(ir012hi, ir012lo, r2, c);
    r3 = r3.wrapping_add(c as u128);  
    ...
    (r2, c) = wadd(ir013lo, ir011hi, r2, c);
    r3 = r3.wrapping_add(c as u128);  

    let r11 = (r1 >> 64) as u64;
    ...
    (_, c) = r11.overflowing_add(m0lo);
    (r2, c) = wadd(ir102lo, ir100hi, r2, c);
